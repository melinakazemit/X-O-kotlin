          ...
          binding.gameStatusText.text =
              when(gameStatus){
                  ...
                  GameStatus.INPROGRESS -> {
                     binding.startGameBtn.visibility = View.GONE //gone or invisibe are both correct
                     currentPlayer + "Turn"
                  }
                  GameStatus.FINISHED -> {
    ...
    override fun onClick(v: View?) {
        gameModel?.apply {
            if (gameStatus != GameStatus.INPROGRESS){
                ...
                return}
                //پیغام نشون میده به کاربر برا 5 ثانیه بعد میره
                //اگه پنجره باز کنه پیغام بده (ارور بده و..) یه مورد دیگه اس
